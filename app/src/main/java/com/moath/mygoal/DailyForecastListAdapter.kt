...
class DailyForecastViewHolder(view: View) : RecyclerView.ViewHolder(view){
    ...
    private val decriptionText: TextView = view.findViewById(R.id.descriptionText)

    fun bind(dailyForecast: DailyForecast) {
        tempText.text = String.format("%.2f", dailyForecast.temp)
        decriptionText.text = dailyForecast.description
    }
}

class DailyForecastAdapter(
    private val clickHandler: (DailyForecast) -> Unit
) : ListAdapter<DailyForecast, DailyForecastViewHolder>(DIEF_CONFIG) {

    companion object {
        val DIEF_CONFIG = object: DiffUtil.ItemCallback<DailyForecast>() {
